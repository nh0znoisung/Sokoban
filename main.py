...
def display_button_A():
    pygame.draw.rect(surface, BLACK, pygame.Rect(
        700 + 275, 305, 95, 48),  0, 6)
    step7Text = buttonFont.render("A*", True, YELLOW_LIGHT)
    surface.blit(step7Text, [700 + 313, 318])


def display_record():
    recordText = recordFont.render(
        "The solution has been recorded in Results folder", True, RED)
...
def display_content_step_3():
    ...
    if not (mode >= 2 and win == 0):
        timeText = helpFont.render(
            "{:0.3f} s".format(timeTook), True, GREEN_LIGHT)
...
def display_undo():
    pygame.draw.rect(surface, RED, pygame.Rect(
        700 + 90, 0 + 410, 50, 40),  0, 20)
    surface.blit(undo_button, [700 + 95, 0 + 410])


def display_redo():
    pygame.draw.rect(surface, RED, pygame.Rect(
        700 + 285, 0 + 410, 50, 40),  0, 20)
...
class Board:
    ...
    def __init__(self):
        ...
        self.name = ''
        # List of tuple (Direction, Boolean), Bool for saving we pushed the boxes or not
        self.history_moves = []
        self.available_moves = []
        # Consider set or 2d-array for time and space-complexity and since the fixed property
        self.walls = set()
        self.goals = set()
        self.boxes = set()
        self.paths = set()
    ...
    def redo(self):
        if self.ptr < len(self.history_moves) - 1:
            self.move(self.history_moves[self.ptr +
                      1].direction, redo=True, move=False)
        # print(self.ptr, len(self.history_moves), self.lose)
        # print(self.is_lose())
    ...
    def minimum_cost(self):
        # add all distances from all goals to all boxes
        temp = []
        for goal in self.goals:
...
def reset_data():
    ...
    goal = pygame.image.load('Items/goal.png')
    player = pygame.image.load('Items/player.png')
    numsRow, numsCol = board.set_value(
        "./Testcases/{}/{}.txt".format(map_list[map_index], level+1))
...
def draw_board(board):
    ...
    for point in board.walls:
        surface.blit(wall, [offsetX + lengthSquare *
                     point.x, offsetY + lengthSquare * point.y])

    for point in board.paths:
        pygame.draw.rect(surface, WHITE, [
                         offsetX + lengthSquare * point.x, offsetY + lengthSquare * point.y, lengthSquare, lengthSquare])
    ...
    for point in board.goals:
        surface.blit(goal, [offsetX + lengthSquare *
                     point.x, offsetY + lengthSquare * point.y])

    point = board.player
    surface.blit(player, [offsetX + lengthSquare *
                 point.x, offsetY + lengthSquare * point.y])

    for point in board.boxes:
        surface.blit(box, [offsetX + lengthSquare * point.x,
                     offsetY + lengthSquare * point.y])
        ...
        for m in moves:
            ...
            if (child not in explored) and child.is_lose() == False:
                ...
                if (child.is_win()):
                    ...
                    print_results(child, node_generated, node_repeated, len(
                        explored), memo_info, timeTook)

                    add_history("Breadth First Search", child.get_history_moves(
                    ), child.step, node_generated, node_repeated, len(explored), memo_info, timeTook)
...
def add_history(algo, sol, ste, gen, rep, expl, memo, dur):
    line_prepender('Results/history_log.txt', algo,
                   sol, ste, gen, rep, expl, memo, dur)
    line_prepender('Results/Solution_{}_test {}'.format(board.name.split('/')
                   [2], board.name.split('/')[3]), algo, sol, ste, gen, rep, expl, memo, dur)
...
def main():
    ...
    while True:
        ...
        if board.is_win() == True and mode == 1:
            ...
            if history == 0:
                add_history("Manually", board.get_history_moves(),
                            board.step, 0, 0, 0, 0, timeTook)
                history = 1
        ...
        print("cost {}".format(board.minimum_cost()))
